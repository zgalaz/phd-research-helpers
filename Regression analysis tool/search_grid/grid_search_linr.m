function res_struct = grid_search_linr(data, clin, sett, search)

% res_struct = grid_search_linr(data, clin, sett, search)
% 
% This function performs a grid search on linear regression (linr) 
% algorithm using predefined parameters. The results of the function are
% stored into  the output structure and can also be stored in *.xlsx file.
%
% INPUT DATA:
...
% sett.algorithm - classification algorithm (predefines: linr)
...
%
% search.modelspec = Model specification
%      {'constant', 'linear', 'interactions', 'purequadratic', ...
%       'quadratic', 'polyijk'} 
% search.intercept = Indicator for constant term
%      {'true', 'false'}
%
%
%
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-linr.xlsx';
    sett.disp      = true;
else
...
if ((nargin < 4) || (isempty(search)))
    search.modelspec  = {'constant', 'linear', 'interactions', ...
                         'purequadratic', 'quadratic', 'polyijk'};
    search.intercept  = {'true', 'false'};
else
    if (~isfield(search, 'modelspec'))
        search.modelspec = {'constant', 'linear', 'interactions', ...
                            'purequadratic', 'quadratic', 'polyijk'};
    end
    if (~isfield(search, 'intercept'))
        search.intercept = {'true', 'false'};
    end
end
...
for p1 = 1:length(sett.g_search{1, 2})
    for p2 = 1:length(sett.g_search{2, 2})
        ...
        reg.modelspec  = sett.g_search{1, 2}{p1};
        reg.intercept  = sett.g_search{2, 2}{p2};
        ...
        for metric_idx = 1:length(sett.metrics)
            ...
            res_table{res_iter + 1, 1} = reg.modelspec;
            res_table{res_iter + 1, 2} = reg.intercept;
