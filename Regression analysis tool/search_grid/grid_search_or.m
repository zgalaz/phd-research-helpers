function res_struct = grid_search_or(data, clin, sett, search)

% res_struct = grid_search_or(data, clin, sett, search)
% 
% This function performs a grid search on ordinal regression (or) 
...
% sett.algorithm - classification algorithm (predefines: or)
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-or.xlsx';
    sett.disp      = true;
else
...
if ((nargin < 4) || (isempty(search)))
    search.link  = {'logit', 'probit', 'comploglog', 'loglog'};
else
    if (~isfield(search, 'link'))
        search.link = {'logit', 'probit', 'comploglog', 'loglog'};
    end
end
...
sett.g_search = {'link', search.link};
...
for p1 = 1:length(sett.g_search{1, 2})
    ...
    reg.link = sett.g_search{1, 2}{p1};

    % Create the display string
    if (sett.disp)
        descript{res_iter} = [sett.g_search{1, 1}, '(', reg.link, ')'];
        
        display  = ['Regressor (or), ', ...
            sett.g_search{1, 1}, '(', ...
                reg.link, ') -> ', ...
        ];
    end
    ...
    for metric_idx = 1:length(sett.metrics)
        ...
        res_table{res_iter + 1, 1} = reg.link;
