function res_struct = grid_search_svmr(data, clin, sett, search)

% res_struct = grid_search_svmr(data, clin, sett, search)
% 
% This function performs a grid search on support vector machines 
% regression (svmr) algorithm using predefined parameters. The results 
% of the function are stored into  the output structure and can also be 
% stored in *.xlsx file.
% 
% INPUT DATA:
...
% sett.algorithm - classification algorithm (predefines: svmr)
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-svmr.xlsx';
    sett.disp      = true;
else
...
if ((nargin < 4) || (isempty(search)))
    search.kernel = {'linear', 'gaussian', 'rbf', 'polynomial'};
else
...
for p1 = 1:length(sett.g_search{1, 2})
    ...
    reg.kernel = sett.g_search{1, 2}{p1};

    % Create the display string
    if (sett.disp)
        descript{res_iter} = [sett.g_search{1, 1}, '(', reg.kernel, ')'];
        
        display = ['Regressor (svmr), ', ...
            sett.g_search{1, 1}, '(', ...
                reg.kernel, ') -> ', ...
        ];
    end
    ...
    for metric_idx = 1:length(sett.metrics)
        ...
        res_table{res_iter + 1, 1} = reg.kernel;
