function [e_train, e_valid] = learning_curve_svmr(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_svmr(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the support vector machines regression 
% algorithm using predefined parameters in order to plot the learning 
% curves (#training examples -> metrics). This enables us to detect if we 
% face the high bias or the high variance ML problem.
% 
% INPUT:
...
% fun_sett.algorithm - classification algorithm (predefines: svmr)
...
%
% ml_sett.kernel = Kernel function
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['SVMR: learning curves (' fun_sett.metrics{i} ')']);
