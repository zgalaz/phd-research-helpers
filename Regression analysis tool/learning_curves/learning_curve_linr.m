function [e_train, e_valid] = learning_curve_linr(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_linr(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the linear regression algorithm using 
% predefined parameters in order to plot the learning curves (#training 
% examples -> metrics). This enables us to detect if we face the high bias 
% or the high variance ML problem.
% 
% INPUT:
...
% fun_sett.algorithm - classification algorithm (predefines: linr)
...
%
% ml_sett.modelspec = Model specification
...
for i = 1:size(X, 1)
    ...
    reg.modelspec = ml_sett.modelspec;
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['LINR: learning curves (' fun_sett.metrics{i} ')']);
