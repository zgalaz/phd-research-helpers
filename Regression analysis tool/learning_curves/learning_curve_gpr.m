function [e_train, e_valid] = learning_curve_gpr(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_gpr(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the Gaussian process regression algorithm 
% using predefined parameters to plot the learning curves (#training 
...
% fun_sett.algorithm - classification algorithm (predefines: gpr)
...
%
% ml_sett.basis = Explicit basis in the GPR model
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['GPR: learning curves (' fun_sett.metrics{i} ')']);
