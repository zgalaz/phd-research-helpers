function [e_train, e_valid] = learning_curve_lda(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_lda(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the linear discriminant analysis (LDA)
% classification algorithm using predefined parameters in order to plot 
% learning curves (#training examples -> metrics). This enables us to 
...
% fun_sett.algorithm - classification algorithm (predefined: lda)
...
% ml_sett.prior  = prior probability for the classes
%               {'empirical', 'uniform'} 
% ml_sett.discrm = discrimination type used in the classifier
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['LDA: learning curves (' fun_sett.metrics{i} ')']);
