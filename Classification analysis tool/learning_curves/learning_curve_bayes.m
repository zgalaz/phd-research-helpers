function [e_train, e_valid] = learning_curve_bayes(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_bayes(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the naive bayes (bayes) classification 
...
% fun_sett.algorithm - classification algorithm (predefined: bayes)
...
% ml_sett.distrb = distribution used in Bayes algorithm
...
if ((nargin < 4) || (isempty(ml_sett)))
    ml_sett.distrb = 'kernel';
    ml_sett.prior  = 'empirical';
else
    if (~isfield(ml_sett, 'distrb'))
        ml_sett.distrb = 'kernel';
    end
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['BAYES: learning curves (' fun_sett.metrics{i} ')']);
