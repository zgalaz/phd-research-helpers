function [e_train, e_valid] = learning_curve_knn(X, X_val, y, y_val, ...
    fun_sett, ml_sett)

% [e_train, e_valid] = learning_curve_knn(X, X_val, y, y_val, ...
%    fun_sett, ml_sett)
% 
% This function trains/validates the k-nearest neighbors classification 
...
% fun_sett.algorithm - classification algorithm (predefined: knn)
...
if ((nargin < 4) || (isempty(ml_sett)))
    ...
    if (~isfield(ml_sett, 'k'))
        ml_sett.k = 2;
    end
    if (~isfield(ml_sett, 'dist'))
        ml_sett.dist = 'euclidean';
    end
end
...
if (fun_sett.plot)
    ...
    for i = 1:length(fun_sett.metrics)
        ...
        title(['KNN: learning curves (' fun_sett.metrics{i} ')']);
