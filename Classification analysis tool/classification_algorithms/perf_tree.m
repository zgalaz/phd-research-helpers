function [c_train, c_test, obj] = perf_tree(d_train, l_train, ...
    d_test, sett, mdl)

% [c_train, c_test] = perf_tree(d_train, l_train, d_test, sett, mdl)
% 
% The functions performs classification (training and testing) of Random
% Forests (Classific. Trees) classifier. This function returns predicted 
% classes for both training and testing set of data. The classifier is 
% specified by setings (sett) structure.
% 
% INPUT DATA:
...
% sett.split = split criterion for ClassificationTree algorithm
%              {'gdi' 'twoing' 'deviance'} 
%              (default: 'gdi')
...
if (~mdl_defined)
    if verLessThan('matlab', '8.4')
        obj = ClassificationTree.fit(d_train, l_train, 'Prior', ...
                sett.prior, 'SplitCriterion', sett.split);
    else
        obj = fitctree(d_train, l_train, 'Prior', ...
                sett.prior, 'SplitCriterion', sett.split);
    end
else
    obj = mdl;
