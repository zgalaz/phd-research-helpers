...
for label_idx  = 1:size(clin_data, 2)
    ...
    for feature_idx = 1:size(feat_data, 2)
        ...
        opt.score      = sett.cls.metrics;
        opt.k_fold     = sett.cv.k_fold;
        opt.disp_stat  = true;
        opt.disp_hist  = true;
        opt.classifier.alg   = sett.cls.algorithm;
        opt.disp_p_progress  = true;
        opt.num_permutations = sett.perm.num_perm;
        opt.num_cross_validations = sett.cv.num_cv;

        p_value = permutation_test(data, clin, opt);
        res_table{feature_idx + 1, end} = p_value;
