function res_struct = grid_search_bayes(data, clin, sett, search)

% res_struct = grid_search_bayes(data, clin, sett, search)
% 
% This function performs a grid search on naive bayes (bayes) classifier
% using predefined parameters. The results of the function are stored into 
% the output structure and can also be stored in *.xlsx file.
%
% INPUT DATA:
...
% sett.algorithm - classification algorithm (predefines: bayes)
...
% search.distrb = distribution used in Bayes algorithm
...
if ((nargin < 3) || (isempty(sett)))
    sett.normalize = true;
    sett.algorithm = 'bayes';
    sett.metrics   = {'mcc', 'acc', 'sen', 'spe'};
    sett.type_cv   = 'KFold';
    sett.num_cv    = 10;
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-bayes.xlsx';
    sett.disp      = true;
else
...
if ((nargin < 4) || (isempty(search)))
    search.distrb = {'normal', 'kernel', 'mvmn', 'mn'};
    search.prior  = {'empirical', 'uniform'};
else
    if (~isfield(search, 'distrb'))
        search.distrb = {'normal', 'kernel', 'mvmn', 'mn'};
    end
    if (~isfield(search, 'prior'))
        search.prior = {'empirical', 'uniform'};
    end
end
...
sett.g_search = {'distrb', search.distrb; ...
                 'prior', search.prior};
...
for p1 = 1:length(sett.g_search{1, 2})
    for p2 = 1:length(sett.g_search{2, 2})
        ...
        cls.distrb = sett.g_search{1, 2}{p1};
        cls.prior  = sett.g_search{2, 2}{p2};
        ...
        if (sett.disp)
            ...
            display  = ['Classifier (bayes), ', ...
                sett.g_search{1, 1}, '(', cls.distrb, '), ', ...
                sett.g_search{2, 1}, '(', cls.prior, ') -> ', ...
            ];
        end
        ...
        for metric_idx = 1:length(sett.metrics)
            ...
            res_table{res_iter + 1, 1} = cls.distrb;
            res_table{res_iter + 1, 2} = cls.prior;
