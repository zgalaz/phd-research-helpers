function res_struct = grid_search_lda(data, clin, sett, search)

% res_struct = grid_search_lda(data, clin, sett, search)
% 
% This function performs a grid search on linear discriminant analysis 
% (lda) classifier using predefined parameters. The results of the 
...
% sett.algorithm - classification algorithm (predefines: lda)
...
%               {'empirical', 'uniform'} 
% search.discrm = discrimination type used in the classifier
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-lda.xlsx';
    sett.disp      = true;
else
...
sett.g_search = {'prior', search.prior; ...
                 'discrm', search.discrm};
...
for p1 = 1:length(sett.g_search{1, 2})
    for p2 = 1:length(sett.g_search{2, 2})
        ...
        cls.prior  = sett.g_search{1, 2}{p1};
        cls.discrm = sett.g_search{2, 2}{p2};
        ...
        if (sett.disp)
            ...
            display  = ['Classifier (lda), ', ...
                sett.g_search{1, 1}, '(', cls.prior, '), ', ...
                sett.g_search{2, 1}, '(', cls.discrm, ') -> ', ...
            ];
        end
        ...
        for metric_idx = 1:length(sett.metrics)
            ...
            res_table{res_iter + 1, 1} = cls.prior;
            res_table{res_iter + 1, 2} = cls.discrm;
