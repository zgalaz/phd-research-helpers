function res_struct = grid_search_knn(data, clin, sett, search)

% res_struct = grid_search_knn(data, clin, sett, search)
% 
% This function performs a grid search on k-nearest neighbors (kNN) 
...
% sett.algorithm - classification algorithm (predefines: knn)
...
% search.k    = the number of nearest neighbors 
% search.dist = distance calculated in knn classifier
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-knn.xlsx';
    sett.disp      = true;
else
...
if ((nargin < 4) || (isempty(search)))
    search.k    = [1:5];
    search.dist = {'euclidean', 'seuclidean', 'cityblock',  ...
                 'chebychev', 'minkowski', 'mahalanobis',   ...
                 'cosine', 'correlation', 'spearman',       ...
                 'hamming', 'jaccard'};
else
...
for p1 = 1:length(sett.g_search{1, 2})
    for p2 = 1:length(sett.g_search{2, 2})
        ...
        cls.k    = sett.g_search{1, 2}(p1);
        cls.dist = sett.g_search{2, 2}{p2};
        ...
        if (sett.disp)
            ...
            display  = ['Classifier (knn), ', ...
                sett.g_search{1, 1}, '(', num2str(cls.k), '), ', ...
                sett.g_search{2, 1}, '(', cls.dist, ') -> ', ...
            ];
        end
        ...
        for metric_idx = 1:length(sett.metrics)
            ...
            res_table{res_iter + 1, 1} = cls.k;
            res_table{res_iter + 1, 2} = cls.dist;
