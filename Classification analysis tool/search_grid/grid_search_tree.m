function res_struct = grid_search_tree(data, clin, sett, search)

% res_struct = grid_search_tree(data, clin, sett, search)
% 
% This function performs a grid search on random forests (tree) classifier
...
% sett.algorithm - classification algorithm (predefines: tree)
...
if ((nargin < 4) || (isempty(search)))
    search.split = {'empirical', 'uniform'};
    search.prior = {'gdi', 'twoing', 'deviance'};
else
    if (~isfield(search, 'split'))
        search.split = {'empirical', 'uniform'};
    end
...
for p1 = 1:length(sett.g_search{1, 2})
    for p2 = 1:length(sett.g_search{2, 2})
        ...
        cls.split = sett.g_search{2, 2}{p2};
        ...
        if (sett.disp)
            ...
            display  = ['Classifier (tree), ', ...
                sett.g_search{1, 1}, '(', cls.prior, '), ', ...
                sett.g_search{2, 1}, '(', cls.split, ') -> ', ...
            ];
        end
        ...
        for metric_idx = 1:length(sett.metrics)
            ...
            res_table{res_iter + 1, 2} = cls.split;
