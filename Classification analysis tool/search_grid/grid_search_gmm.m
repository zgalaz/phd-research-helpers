function res_struct = grid_search_gmm(data, clin, sett, search)

% res_struct = grid_search_gmm(data, clin, sett, search)
% 
% This function performs a grid search on gaussian mixture models (gmm) 
% classifier using predefined parameters. The results of the function are 
...
% sett.algorithm - classification algorithm (predefines: gmm)
...
% search.mixt   = number of Gaussian mixtures 
%
%
%
% --
...
if ((nargin < 3) || (isempty(sett)))
    ...
    sett.k_fold    = 10;
    sett.savetable = true;
    sett.tablename = 'grid_search-gmm.xlsx';
    sett.disp      = true;
else
...
sett.g_search = {'mixt', search.mixt};
...
for p1 = 1:length(sett.g_search{1, 2})
    ...
    cls.mixt = sett.g_search{1, 2}(p1);

    % Create the display string
    if (sett.disp)
        descript{res_iter} = [sett.g_search{1, 1}, '(', ...
            num2str(cls.mixt), ')'];
        
        display  = ['Classifier (gmm), ', ...
            sett.g_search{1, 1}, '(', num2str(cls.mixt), ') -> ', ...
        ];
    end
    ...
    for metric_idx = 1:length(sett.metrics)
        ...
        res_table{res_iter + 1, 1} = cls.mixt;
